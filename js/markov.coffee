...
svg.append('svg:defs').append('svg:marker')
  ...
  .attr('refX', 6).attr('markerWidth', 4)
  .attr('markerHeight', 4)
...
restart = ->
  ...
  path
    .style('stroke', "black" )
    .style('fill', "black" )
    .style('stroke-width', (d) -> d.poids+'px') 
  ...
  circle.selectAll('circle')
    .style('fill', (d) -> if d == selected_node then d3.rgb(colors(d.id)).brighter().toString() else d3.rgb(colors(d.id)))
