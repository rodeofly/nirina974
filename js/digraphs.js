...
// set up SVG for D3
var DnDFileController, circle, circlesGroup, colors, dnd, drag_line, force, height, i, j, keydown, keyup, lastKeyDown, lastNodeId, links, mousedown, mousedown_link, mousedown_node, mousemove, mouseup, mouseup_node, n, nodes, path, pathsGroup, ref, resetMouseVars, restart, save, selected_link, selected_node, sgt, spliceLinksForNode, spoiler, statut, svg, tick, width;
...
restart = function() {
  ...
  for (m = 0, len1 = nodes.length; m < len1; m++) {
    sommet = nodes[m];
    e = 0;
    for (o = 0, len2 = links.length; o < len2; o++) {
      arete = links[o];
  ...
  for (passage = p = 0, ref1 = lastNodeId; (0 <= ref1 ? p < ref1 : p > ref1); passage = 0 <= ref1 ? ++p : --p) {
    for (q = 0, len3 = nodes.length; q < len3; q++) {
      sommet = nodes[q];
      [e, s] = [0, 0];
  ...
  $("#departs").empty();
  $("#arrivees").empty();
  for (u = 0, len5 = nodes.length; u < len5; u++) {
    sommet = nodes[u];
    sommet.reflexive = false;
  }
  for (v = 0, len6 = nodes.length; v < len6; v++) {
    sommet = nodes[v];
    $("#sommets").append(`<td>${sommet.id}</td>`);
    [e, s] = [0, 0];
    for (w = 0, len7 = links.length; w < len7; w++) {
      arete = links[w];
      if (arete.source === sommet && arete.right) {
        s += 1;
      }
