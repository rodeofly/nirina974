...
var DnDFileController, autre, circle, circlesGroup, couleurs, dnd, drag_line, force, height, jeu, joueur, keydown, keyup, lastKeyDown, lastNodeId, links, mousedown, mousedown_link, mousedown_node, mousemove, mouseup, mouseup_node, nodes, path, pathsGroup, premier_appel, resetMouseVars, restart, save, selected_link, selected_node, spliceLinksForNode, svg, tick, width,
  indexOf = [].indexOf;
...
svg.append('svg:defs').append('svg:marker').attr('id', 'end-arrow').attr('viewBox', '0 -8 16 16').attr('refX', 12).attr('markerWidth', 6).attr('markerHeight', 6).attr('orient', 'auto').append('svg:path').attr('d', 'M0,-8L16,0L0,8').attr('fill', '#000');

svg.append('svg:defs').append('svg:marker').attr('id', 'start-arrow').attr('viewBox', '0 -8 16 16').attr('refX', 8).attr('markerWidth', 6).attr('markerHeight', 6).attr('orient', 'auto').append('svg:path').attr('d', 'M16,-8L0,0L16,8').attr('fill', '#000');
...
restart = function() {
  ...
  path.style('stroke', function(d) {
    ...
    return d.color;
  }).style('stroke-width', '4px').classed('selected', function(d) {
  ...
  // add new links
  path.enter().append('svg:path').attr('class', 'link').style('stroke', function(d) {
    return d.color;
  }).style('fill', function(d) {
    return d.color;
  }).style('stroke-width', '4px').classed('selected', function(d) {
    ...
    if (d3.event.ctrlKey || jeu) {
      ...
      selected_node = null;
      if (selected_link && jeu && (ref = selected_link.color, indexOf.call(couleurs[joueur], ref) >= 0)) {
  ...
  g.append('svg:circle').attr('class', 'node').attr('r', 12).style('fill', "white").style('stroke', "black").classed('pion', function(d) {
    return d.pion;
  }).classed('arrivee', function(d) {
    return d.arrivee;
  }).classed('depart', function(d) {
